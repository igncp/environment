...
pub fn setup_pi_hole(context: &mut Context) {
    ...
    context.system.install_system_package("unbound", None);

    std::fs::create_dir_all(context.system.get_home_path(".pi-hole")).unwrap();

    context.files.append(
        &context.system.get_home_path(".pi-hole/docker-compose.yml"),
        r###"
version: "3"
