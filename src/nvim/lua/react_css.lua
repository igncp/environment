...
function OpenCorrespondingReactSCSS(search)
  ...
  if search then
    word_to_search = vim.fn.expand('<cword>') or ""
    if word_to_search == "styles" then
      vim.cmd(M.t('normal! f.l') or "")
      word_to_search = vim.fn.expand('<cword>') or ""
    end
  end

  local file_name = vim.fn.expand('%:t') or ""
...
vim.api.nvim_set_keymap("n", "<leader>mo",
  "<cmd>lua OpenCorrespondingReactSCSS(true)<CR>",
  { noremap = true })
vim.api.nvim_set_keymap("n", "<leader>me",
  "<cmd>lua OpenCorrespondingReactSCSS(false)<CR>",
  { noremap = true })
...
function AddStylesImport(is_common)
  ...
  if string.match(file_name, '.*%.tsx') then
    ...
    elseif is_common then
      vim.cmd(
        'normal! ggOimport * as commonStyles from \'~/styles/common.module.scss\';\r')
    else
      vim.cmd('normal! ggOimport * as styles from \'./' .. file_name_base ..
        '.module.scss\';\r')
    end
    vim.cmd('normal! ``')
  elseif string.match(file_name, '.*%.module.scss') then
    local function search_item() vim.cmd('normal! /sass.scss\r') end
    if pcall(search_item) then
      -- Do nothing if the import already exists
    else
